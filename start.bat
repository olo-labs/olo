...
REM Active pipeline definitions (olo-mono/olo-configuration/current-active/)
if not defined OLO_CONFIGURATION_DIR (
  set "OLO_CONFIGURATION_DIR=%~dp0..\olo-mono\olo-definition\olo-configuration\current-active"
)
...
if not exist "%OLO_CONFIGURATION_DIR%" (
  ...
  echo.
  echo Ensure current-active exists under olo-mono/olo-definition/olo-configuration, or set OLO_CONFIGURATION_DIR.
  pause
  exit /b 1
)
